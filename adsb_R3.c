...
int main(int argc, char *argv[]) {
    ...
    item *result[SPLIT];
    int dx, ds, previous;
    int forward_xs[50001], backward_xl[50001], forward_yl[50001],
        backward_ys[50001];
    ...
    for (i = 0; i < SPLIT; i++) {
        if (i == SPLIT - 1) {
            result[i] =
                editdis_min_all(rx + dx, PART, rl + ds, strlen(rl) - ds);
            break;
        }
    ...
    for (i = 0; i < SPLIT; i++) {
        if (i == SPLIT - 1) {
            result[i] =
                editdis_min_all(ry + dx, PART, rs + ds, strlen(rs) - ds);
            break;
        }
    ...
    int tmp2;
    for (i = 1; i <= 50000; i++) {
        int tmp = forward_xs[i] + backward_xl[50001 - i] + forward_yl[i] +
                  backward_ys[50001 - i];
